...
class PlotBase:
    ...
    def _gen_data(
        ...
        eps_m = np.append(
            eps_m_lt, np.linspace(self.eps_sr_cr + delta_eps_m_lt, eps_m_max)
        )
...
class PlotMTCM(PlotBase):
    def chord_distribution(self):
        ...
        try:
            ...
            print(
                f'ERROR: The RC tie is in "{self.condition} condition".'
                f'Plot of chord distributions is only possible for '
                f'Regime 1, i.e. for tensile stresses at crack below yielding. '
            )
    ...
    def plot_miso(self):
        ...
        fig.add_trace(
            go.Scatter(
                x=self.eps_m_vector, y=self.sigma_sr_mtcm_strain, name="MTCM"
            )
        )
        ...
        fig.add_trace(
            go.Scatter(x=self.eps_m_miso, y=self.sigma_sr_miso, name="MISO")
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=self.eps_m_miso, y=self.sigma_sr_miso, name="Total strains"
            )
        )
